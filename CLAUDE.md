...
- **`state/`** — `constants.js` (NODE_RADIUS=20, COLORS_LIGHT/DARK palettes, node/edge material/type/diameter catalogs, `isDarkMode()`), `persistence.js` (IndexedDB ↔ localStorage bridging, STORAGE_KEYS), `placement-mode.js` (rapid placement: when on, tapping empty canvas places a node without opening the field stepper — one tap per manhole instead of two; toggled from the node-mode flyout, persisted in localStorage), `app-state.js`, `app-store.js`, `event-bus.js`, `skill-level.js`
...
**User-facing tutorials** live in [`docs/tutorials/`](./docs/tutorials/README.md) (8 illustrated guides). Their screenshots are generated by `scripts/docs-screenshots.mjs` against the local dev server with a fully mocked backend (never production) — regenerate them after UI changes: `npm run dev` + `node scripts/docs-screenshots.mjs`.
