...
All slope/length/Z math goes through the existing pure functions — `computeEdgeGradient` (accepts hypothetical `{tail, head, tail_measurement:'', head_measurement:''}` edges), `edgeLengthM`, `elevationOf` (Z=0 sentinel) in `features/gradient-engine.js`. All mutations go through `F.createNode` / `F.createEdge` / new `F.reverseEdge` / new `F.splitEdge` / new `F.attachMeasurementToNode` so the read-only guard, admin defaults, undo-redo, and gradient hooks come free.
...
6. **Tier decision** by the suggestion engine (`suggestConnections(newNode, ctx)`):
...
7. **CONNECT screen** *(the one new window — implemented as a field-stepper screen type, prepended to `getScreenSequence()`)*. One screen hosting, in fixed order, only the cards that fired:
   ...
   - **primary direction card** (B3/B3a/B3b) with evidence line and swap chip
...
| **B2** | Confident chain auto-connect | isNew AND previous chain node is ALSO the top spatial candidate AND distance ≤ 70 m AND both `surveyZ` truthy AND \|ΔZ\| > `TOL_TERRAIN_M` (0.05) AND downhill direction is unambiguous AND no edge either direction AND nodeType ∉ {Home} AND stepper not busy elsewhere | Create edge silently, **tail = higher-Z node** (this may flip chronological order) | Success snackbar "חיבור 104 → 105 · ‎1.2% · 38 מ'" + Undo | AUTO |
...
| **B3a** | Flat/ambiguous ΔZ | \|ΔZ\| ≤ 0.05 or terrain slope < `MIN_SLOPE_PCT` | Never guess. Two equal-weight direction chips + "אחליט בעומקים" (creates chronological direction, `direction_source:'chronological'`, re-asked at depth entry) | Same screen, no pre-selection, helper line "שטוח: הפרש 0.03 מ'" | ASK |
...
|---|---|---|
| `edge.direction_source` | `'chronological' \| 'terrain' \| 'invert' \| 'user'` | Provenance for every arrow. Depth-time rechecks (B9) nag guessed directions once and never nag user-confirmed ones; C6 uses it to decide whether to offer flip. Snake_case chosen to match the `tail_measurement` family. (Naming conflict resolved: enterprise's `directionSource` loses.) |
| `F.reverseEdge(edgeId)` — **operation** | swap `tail`↔`head` AND `tail_measurement`↔`head_measurement`, keep `fall_depth` (re-ask `fall_position` if set), undo entry, gradient recheck | Prerequisite for B4, B9, B10, C6, C8. Nothing like it exists (grep-confirmed); also closes the standing edgeExists backlog item. |
...
| **Not added:** per-node IL, rim, multi-shot history | — | Invert stays derived (`surveyZ − depth` per edge end) — matches the engine's two-basis design; adding stored IL would create a second source of truth. |

---

## 5. Phased implementation plan

### Phase 1 — direction-correct chaining + ping-pong fix (highest value, smallest surface)
- `features/gradient-engine.js`: no changes needed yet — reuse `computeEdgeGradient`/`edgeLengthM`/`elevationOf` from a new sibling module `features/connection-suggest.js` (pure: `suggestConnections(newNode, ctx) → [{kind, tail, head, evidence, tier}]`).
...
- i18n keys (he+en) in `i18n.js`; e2e scenarios (downhill chain, uphill flip, flat ask) in `frontend/tests/e2e/tsc5-field-workflows.spec.ts` + `scripts/mock-tsc3/server.mjs`.

### Phase 2 — identity guards + candidates
