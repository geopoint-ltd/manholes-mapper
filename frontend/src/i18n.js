...
export const i18n = {
  he: {
    ...
    modeRapidPlace: 'רצף',
    modeRapidPlaceHint: 'הנחה מהירה — בלי לפתוח את הטופס',
    home: 'בית',
    drainage: 'קולטן',
    ...
    toasts: {
      ...
      nodeTooCloseSelected: 'נבחרה השוחה הסמוכה — התקרב כדי להוסיף שוחה כאן',
      rapidPlacementOn: 'הנחה מהירה פעילה — הקש על שוחה כדי למלא פרטים',
      rapidPlacementOff: 'הנחה מהירה כבויה',
      edgeDeleted: 'הקו נמחק',
      edgeExists: 'כבר קיים קו בין השוחות הללו',
      undoEdgeReverse: 'כיוון הקו הוחזר',
      redoEdgeReverse: 'כיוון הקו הוחלף שוב',
...
התא השכן יוסר והחיבורים שלו יועברו לשרטוט הפעיל.`,
    ...
    survey: {
      ...
      autoConnected: (tail, head) => `חובר קו ⁦${tail} → ${head}⁩`,
      slopePart: (pct) => `שיפוע ${pct}%`,
      lengthPart: (m) => `${m} מ'`,
      autoConnectUndo: 'ביטול',
    ...
    stepper: {
      closeAria: 'סגירת עורך השדות',
      stepOf: (i, n) => `שלב ${i} מתוך ${n}`,
      currentValueLabel: 'ערך נוכחי',
      noValueYet: 'טרם נקבע',
      next: 'הבא',
      back: 'הקודם',
      skip: 'דלג',
      deadEndExplainer: 'יש לענות תחילה על סטטוס תחזוקה כדי לפתוח את שאר השדות',
      depthsTitle: 'עומקי קווים מחוברים',
      depthsNone: 'אין קווים מחוברים לצומת זה',
      depthTail: 'עומק בקצה הזנב',
      depthHead: 'עומק בקצה הראש',
      depthRangeWarning: 'עומק חריג לטווח הרגיל (0–15 מ\')',
      edgeDetailsToggle: 'פרטי קו נוספים',
      completionTitle: 'סיכום מילוי',
      completionSummary: (pct, filled, total) => `הושלמו ${pct}% (${filled} מתוך ${total} שדות)`,
      completionSkipped: (n) => `${n} שדות דולגו`,
      saveNext: 'שמור והמשך לבא',
      doneClose: 'סיום',
      tsc3QueuedBody: (pointName) => `נקודת מדידה ${pointName} התקבלה בזמן שמילאת נתונים לצומת אחר`,
      tsc3QueuedAction: 'עבור לנקודה',
      // CONNECT screen — ⁦..⁩ (LRI/PDI) keep latin id fragments LTR inside RTL text
      connectTitle: 'חיבור לרשת',
      connectQuestion: (id) => `לחבר אל ⁦${id}⁩?`,
      connectFlat: (dz) => `הפרש גבהים ${dz} מ' — שטוח, בחר כיוון זרימה`,
      connectNoZ: 'חסר גובה — הכיוון לפי סדר המדידה בלבד',
      connectFar: (m) => `מרחק ${m} מ' מהנקודה הקודמת — כנראה קטע חדש`,
      connectRecommended: 'מומלץ',
      connectUnverified: 'לא מאומת',
      connectSlopeOk: (pct) => `שיפוע ${pct}% ✓`,
      connectSlopeUp: (pct) => `עולה ${pct}% ⚠`,
      connectNone: 'ללא חיבור',
      connectFlipQuestion: (tail, head, pct) => `הקו ⁦${tail} → ${head}⁩ עולה בכיוון הזרימה (${pct}%) — להפוך את הכיוון?`,
      connectFlip: 'הפוך כיוון',
      connectKeep: 'השאר כמו שהוא',
    },
  },
  en: {
    ...
    modeRapidPlace: 'Rapid',
    modeRapidPlaceHint: 'Rapid placement — place without opening the form',
    home: 'Home',
    drainage: 'Drainage',
    forLater: 'For Later',
    ...
    toasts: {
      ...
      nodeTooCloseSelected: 'Selected the nearby manhole — zoom in to place one here',
      rapidPlacementOn: 'Rapid placement on — tap a manhole to fill it in',
      rapidPlacementOff: 'Rapid placement off',
      edgeDeleted: 'Edge deleted',
      edgeExists: 'An edge between these nodes already exists',
      undoEdgeReverse: 'Edge direction restored',
      redoEdgeReverse: 'Edge direction flipped again',
    ...
    survey: {
      ...
      autoConnected: (tail, head) => `connected ${tail} → ${head}`,
      slopePart: (pct) => `slope ${pct}%`,
      lengthPart: (m) => `${m}m`,
      autoConnectUndo: 'Undo',
    ...
    stepper: {
      closeAria: 'Close field editor',
      stepOf: (i, n) => `Step ${i} of ${n}`,
      currentValueLabel: 'Current value',
      noValueYet: 'Not set yet',
      next: 'Next',
      back: 'Back',
      skip: 'Skip',
      deadEndExplainer: 'Answer maintenance status first to unlock the remaining fields',
      depthsTitle: 'Connected pipe depths',
      depthsNone: 'No pipes connected to this node',
      depthTail: 'Tail-end depth',
      depthHead: 'Head-end depth',
      depthRangeWarning: 'Depth outside the usual 0–15m range',
      edgeDetailsToggle: 'More pipe details',
      completionTitle: 'Fill summary',
      completionSummary: (pct, filled, total) => `${pct}% complete (${filled} of ${total} fields)`,
      completionSkipped: (n) => `${n} fields skipped`,
      saveNext: 'Save & Next',
      doneClose: 'Done',
      tsc3QueuedBody: (pointName) => `Survey point ${pointName} arrived while filling another node`,
      tsc3QueuedAction: 'Switch to it',
      connectTitle: 'Connect to network',
      connectQuestion: (id) => `Connect to ${id}?`,
      connectFlat: (dz) => `Elevation difference ${dz}m — flat, choose the flow direction`,
      connectNoZ: 'Missing elevation — direction from measurement order only',
      connectFar: (m) => `${m}m from the previous point — probably a new run`,
      connectRecommended: 'Recommended',
      connectUnverified: 'Unverified',
      connectSlopeOk: (pct) => `slope ${pct}% ✓`,
      connectSlopeUp: (pct) => `uphill ${pct}% ⚠`,
      connectNone: 'No connection',
      connectFlipQuestion: (tail, head, pct) => `Pipe ${tail} → ${head} runs uphill (${pct}%) — flip the direction?`,
      connectFlip: 'Flip direction',
      connectKeep: 'Keep as is',
    },
  },
};
